...
The proposed framework measures the similarity between documents to decide whether or not they share the same class, even if that class has not been seen during training. In essence, the proposed \gls{VDM} method frames the zero-shot identification problem as a binary matching problem, where the system determines if a pair of documents is visually equivalent (match or no-match visually).
